...
export default function LiveTrackingPage() {
  ...
  // Initialize Google Map directly (no LoadScript — uses the script loaded in index.html)
  useEffect(() => {
    const initMap = () => {
    ...
    initMap();
    const t1 = setTimeout(initMap, 500);
    const t2 = setTimeout(initMap, 1500);
    const t3 = setTimeout(initMap, 3000);
  ...
  // Update map markers and polylines when shipments or selection changes
  useEffect(() => {
    if (!mapRef.current || !mapReady) return;
    const map = mapRef.current;
    const iw = infoWindowRef.current;

    // Track which markers/polylines are still needed
    ...
    filtered.forEach(s => {
      ...
      // Determine marker position: use current_lat/lng if available, else origin
      const markerLat = s.current_lat || s.origin_lat;
      const markerLng = s.current_lng || s.origin_lng;

      // Draw route polyline (origin → destination)
      if (s.origin_lat && s.destination_lat) {
        if (polylinesRef.current[s.id]) {
          // Update existing polyline
          polylinesRef.current[s.id].setOptions({
            strokeColor: color,
          ...
          polylinesRef.current[s.id] = polyline;
        }
      }
      ...
      if (markerLat) {
        if (markersRef.current[s.id]) {
          // Update existing marker position (smooth for live tracking)
          markersRef.current[s.id].setPosition({ lat: markerLat, lng: markerLng });
          markersRef.current[s.id].setIcon({
            path: window.google.maps.SymbolPath.CIRCLE,
            fillColor: color,
            fillOpacity: 0.9,
            strokeColor: isSelected ? '#ffffff' : 'rgba(255,255,255,0.5)',
            strokeWeight: isSelected ? 3 : 1,
            scale: isSelected ? 11 : (isLive ? 9 : 7),
          });
        } else {
          // Create new marker
          ...
          marker.addListener('click', () => {
            ...
            const content = `
              <div style="min-width:180px;font-family:system-ui,sans-serif">
                <div style="font-family:monospace;font-size:10px;color:#94a3b8;margin-bottom:2px">${s.id?.substring(0,12)}</div>
                <strong style="color:#0f172a;font-size:13px">${s.origin_name} → ${s.destination_name}</strong>
                <div style="margin-top:6px;display:flex;gap:8px;flex-wrap:wrap">
                  <span style="font-size:11px;background:#f4f6fb;border-radius:4px;padding:2px 6px;color:${color};font-weight:700">${s.risk_level} risk</span>
                  <span style="font-size:11px;color:#64748b">${(s.progress_pct||0).toFixed(0)}% done</span>
                </div>
                ${s.current_speed_kmh ? `<div style="margin-top:4px;font-size:11px;color:#64748b">${s.current_speed_kmh.toFixed(0)} km/h</div>` : ''}
              </div>`;
    ...
    if (selected) {
      const lat = selected.current_lat || selected.origin_lat;
      const lng = selected.current_lng || selected.origin_lng;
      if (lat && lng) {
        map.panTo({ lat, lng });
        if (map.getZoom() < 7) map.setZoom(7);
      }
    }
  }, [filtered, selectedId, mapReady, activeSimulations]);
        ...
        <div className="map-container glass-card" style={{padding:0,overflow:'hidden'}}>
          ...
          {!mapReady && (
            <div style={{
              ...
              alignItems: 'center', justifyContent: 'center', background: 'rgba(248,250,252,0.9)',
              zIndex: 10,
            }}>
              <div className="loader-spinner" style={{width:32,height:32}}/>
              <p style={{marginTop:'0.75rem',fontSize:'0.875rem',color:'var(--text-muted)'}}>Loading map…</p>
            </div>
          )}
